...
public class SnakeAndLadderService {
    ...
    private Queue<Player> playerQueue; // Comment: Keeping players in game service as they are specific to this game and not the board. Keeping pieces in the board instead.
    ...
    public void setPlayerQueue(List<Player> playerQueue) {
        this.playerQueue = new LinkedList<Player>();
        this.initialNumberOfPlayers = playerQueue.size();
    ...
    public void startGame() {
        while (!isGameCompleted()) { // checks based on the number of players
            int totalDiceValue = getTotalValueAfterDiceRolls(); // Each player rolls the dice when their turn comes.
            Player currentPlayer = playerQueue.poll(); // get the current player
            movePlayer(currentPlayer, totalDiceValue); // move the player, also considers the snakes and ladders
            if (hasPlayerWon(currentPlayer)) { // if the player has won then remove his piece and also don't add him to the queue
                System.out.println(currentPlayer.getName() + " wins the game");
                snakeAndLadderBoard.getPlayerPieces().remove(currentPlayer.getId());
            } else {
                playerQueue.add(currentPlayer); // if player didnt win, add him to the queue
            }
        }
    }
}
